...
class BitQuantizer:
    ...
    def __init__(self, model, n_fs, n_bits, layers_qual_path=None, avoid=[], verbose=2):
        ...
        for layer_number, layer_path in enumerate(self.layers_qual_path):
            ...
            if not (
                isinstance(model_layer, nn.Conv2d) or isinstance(model_layer, nn.Linear)
            ):
                continue
    ...
    def init_hash_functions(self):
        ...
        for layer_data in self.layer_datas:
            ...
            layer_data.fs = torch.from_numpy(
                np.repeat(coefs.reshape(1, -1), layer_n_fs, axis=0)
            ).cuda()
                ...
                for _ in tqdm(range(n_iter), disable=disable_tqdm):
                    ...
                    for fidx in range(0, layer_data.n_fs):
                        ...
                        if fidx != layer_data.n_fs - 1:
                            wx = w_master[
                                fidx * channel_step : (fidx + 1) * channel_step
                            ]  # [channel_step,inc,k,k]
                        else:
                            wx = w_master[
                                fidx * channel_step :
                            ]  # [channel_step,inc,k,k]
                        ...
                        if w.size()[0] == 0:
                            raise Exception(
                                f"Difference between quant_levels and weight is of shape {w.size()}."
    ...
    def get_hashed_model(self):
        ...
        for layer_data in self.layer_datas:
            getattr_by_path_list(model, layer_data.qual_path).weight = nn.Parameter(
                layer_data.hashed_weight
            )
        return model
...
def use_hashed_conv(model, n_bits=6, n_functions=64):
    ...
    for layer, layer_path in conv_layers_with_path:
        kwargs = {attr: copy.deepcopy(getattr(layer, attr)) for attr in conv_attrs}
