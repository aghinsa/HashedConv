...
class HashedConv2d(nn.Module):
    ...
    def forward(self, x, x_hash):
        ...
        self.fx = torch.add(
            torch.mm(self.a1, cw) + torch.mm(self.a2, torch.pow(cw, 2)), self.a0
        )
        self.hash_weight = torch.mm(
            self.softmax(self.fx * self.T).t(), self.values
        ).reshape(N, C, H, W)
...
class HashedLinear(nn.Module):
    ...
    def forward(self, x, x_hash):
        ...
        self.fx = torch.add(
            torch.mm(self.a1, fcw) + torch.mm(self.a2, torch.pow(fcw, 2)), self.a0
        )
        self.hash_weight = torch.mm(
            self.softmax(self.fx * self.T).t(), self.values
        ).reshape(N, M)
