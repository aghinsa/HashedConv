...
class HashedConv(nn.Conv2d):
    ...
    def __init__(self,*args,**kwargs):
        ...
        self.n_bins = 16
        self.bins = nn.Parameter(torch.rand(self.n_bins,1))
        self.probs = nn.Parameter(torch.randn(self.weight.size() + (self.n_bins) ))
        print(f"SElf probs : {self.probs.size()}")
        self.weight = None
        self.softmax = nn.Softmax(dim=-1)
    def forward(self,x):
        new_weight = (self.softmax(self.probs)*self.bins()).sum(dim=-1)
        print(f"new_weight : {new_weight.size()}")
